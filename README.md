...
`public/js/auto.js` ไล่ทำตารางทีละตัวด้วย **ขั้นตอนเดิมครบทุกขั้น** ผ่าน endpoint เดิม —
ไม่ได้ขอ bulk operation จากเซิร์ฟเวอร์ กติกา one-table-at-a-time ยังอยู่ครบ ทุกตารางได้ preflight gate
baseline แผน และ job ของตัวเอง สิ่งที่หายไปคือการกดเท่านั้น

สิ่งเดียวที่คนทำแล้วโค้ดทำแทนไม่ได้คือ **อ่านแผน** ตรงไหนที่ต้องใช้วิจารณญาณ มันจะข้ามแทน:
...
**ทำไมยกเว้น `lossy_narrowing`** — risk ตัวนี้ขึ้นกับทุกตารางที่แปลง utf8mb4 → utf8mb3 เพราะนั่นคือ
งานของเครื่องมือนี้ มันบอกว่า "อักขระนอกขอบเขตจะกลายเป็น `?` ต้องสแกนก่อน" ซึ่งตอนที่แผนถูกสร้าง
...
ใส่ `0` = ไม่จำกัดด้านนั้น

**ตัวเลือกขั้นสูงชุดเดียวกับ 5 ขั้น** — สิ่งที่โผล่บนหน้าจอคือ 4 ค่าที่ตัดสินว่า*ตารางไหน*จะรัน
ส่วนสิ่งที่ตัดสินว่าแต่ละตาราง*รันยังไง* อยู่ใต้ **"ตัวเลือกขั้นสูง"** อันเดียวกับที่หน้าทีละตารางใช้
เรียงตามขั้นที่มันทำงาน และตั้งต้นไว้ตรงกับค่าที่ตัวรันเคย hard-code ไว้ทุกตัว —
รันโดยไม่แตะอะไรเลยจึงได้พฤติกรรมเดิมเป๊ะ (มีเทสต์ยืนยันค่าตั้งต้นทั้งชุด)

| ขั้น | ตั้งได้ | ตั้งต้น |
|---|---|---|
| 1 ตรวจข้อมูล | ดูกี่แถว · เก็บตัวอย่างกี่แถว · เช็ค UNIQUE · เช็ค double-encoding | เพดานสแกนของเซิร์ฟเวอร์ · 5 · เปิด · เปิด |
| 2 เก็บ baseline | อ่านแค่ไหน · วิธี hash · แยกทีละคอลัมน์ | auto · sha256 · ปิด |
| 4 รันคำสั่ง | checksum ก่อน/หลัง · ย้อนกลับอัตโนมัติ · ไม่รอเซิร์ฟเวอร์ว่าง | เปิด · เปิด · ปิด |
| 5 เทียบผล | — (ใช้วิธีเดียวกับ baseline เสมอ) | — |

สามตัวในขั้น 4 คือพวกที่ **ปิดการตรวจ** ไม่ใช่แค่ปรับความละเอียด ถ้าตัวไหนถูกปิด
กล่องยืนยันจะขึ้นเป็น critical พร้อมรายชื่อ และเปลี่ยนคำที่ต้องพิมพ์จาก `RUN` เป็น `FORCE`
และค่าที่ต่างจากตั้งต้นจะค้างอยู่บนการ์ดความคืบหน้าตลอดการรัน — ตัวเลือกที่เห็นแค่ในกล่องยืนยัน
คือตัวเลือกที่อีกชั่วโมงต่อมาไม่มีใครตรวจย้อนได้ ส่วน `stopOnError` ไม่เปิดให้ตั้ง: การรันต่อหลัง step
ล้มเหลวคือการรันที่ไม่มีใครดู ทำสิ่งที่มันเพิ่งพิสูจน์แล้วว่าทำไม่ได้ และ checksum ที่ job เก็บเองก่อน/หลัง
ยังคงเป็น `auto` เสมอไม่ว่า baseline จะตั้งเป็นอะไร เพราะสองตัวนั้นรันอยู่ใน**ช่วงที่ตารางล็อก**
